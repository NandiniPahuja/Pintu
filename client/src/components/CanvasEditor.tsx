...
export default function CanvasEditor() {
  ...
  useEffect(() => {
    if (canvasRef.current && !canvas) {
      ...
      // Enable all control points (8 scaling corners + rotation)
      ...
      // Add mouse tracking for better drag behavior
      let isDragging = false
      let dragStartX = 0
      let dragStartY = 0
      let dragTarget: fabric.Object | null = null
      ...
      fabricCanvas.on('mouse:down', (e) => {
        ...
        // If clicking outside while editing text, exit editing
        if (activeObject && (activeObject as any).isEditing && !e.target) {
      ...
      fabricCanvas.on('mouse:move', (e) => {
        if (isDragging && dragTarget) {
          ...
          const currentLeft = dragTarget.left || 0
          const currentTop = dragTarget.top || 0
      ...
      fabricCanvas.on('mouse:dblclick', (e) => {
        ...
        // Check if the target is a text object (IText or Textbox)
        if (target && (target instanceof fabric.IText || target instanceof fabric.Textbox)) {
          ...
          // Ensure the canvas renders the editing state
          fabricCanvas.renderAll()
          
          console.log('Entered editing mode for text:', target.text)
        }
      })
      ...
      const handleKeyDown = (e: KeyboardEvent) => {
        ...
        // Escape key - Exit text editing mode
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      fabric.Image.fromURL(imgUrl, (img) => {
        ...
        // Enable all control points for image
    ...
    try {
      ...
      if (words.length > 0) {
        words.forEach((word: any, wordIndex: number) => {
          ...
          if (word.text.trim() && word.confidence > 20 && word.bbox) {
            ...
            // Create editable text box with professional styling
            // Using Textbox instead of IText for better in-place editing
            const textObj = new fabric.Textbox(word.text, {
              left: textLeft,
              top: textTop,
              width: textWidth, // Set width for proper text wrapping
              fontSize: fontSize,
        ...
        if (wordCount > 0) {
          canvas.renderAll()
          updateLayers()
          alert(`✅ Extracted ${wordCount} text boxes!\n\n📍 Text positioned at exact source locations\n✏️ DOUBLE-CLICK any text to edit in-place\n🎨 Drag, resize, or rotate text elements\n⌨️ Press ESC to exit editing mode\n👁️ Toggle "Show/Hide Image" for better editing\n\n💡 Pro tip: Each text box is fully editable - just double-click and start typing!`)
        }
      }
      ...
      if (wordCount === 0 && lines.length > 0) {
        ...
        if (wordCount > 0) {
          canvas.renderAll()
          updateLayers()
          alert(`✅ Extracted ${wordCount} text boxes!\n\n📍 Text positioned at exact locations\n✏️ DOUBLE-CLICK to edit in-place\n⌨️ Press ESC to exit editing\n👁️ Click "Show/Hide Image" for clearer view\n\n💡 Each text box is fully editable!`)
        }
      }
  ...
  return (
    <div className="flex h-screen bg-gray-50">
      ...
      <div className="w-64 bg-white border-r border-gray-200 p-4 space-y-3 overflow-y-auto">
        ...
        <div className="border-b pb-3 mb-3">
          <button
            ...
            className="w-full bg-black text-white px-4 py-2 rounded-lg hover:bg-gray-900 font-mono font-medium mb-2 transition-colors"
          >
            📤 Upload Image
          </button>
          ...
          {uploadedImage && (
            <>
              <button
                ...
                className="w-full bg-gray-800 text-white px-4 py-2 rounded-lg hover:bg-gray-900 font-mono font-medium mb-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
              ...
              <button
                ...
                className="w-full bg-white border-2 border-gray-800 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-50 font-mono font-medium text-sm transition-colors"
              >
                👁️ Show/Hide Image
              </button>
            </>
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={addText}
            className="w-full bg-gray-800 text-white px-4 py-2 rounded-lg hover:bg-black text-sm font-mono font-medium transition-colors"
          >
            ✏️ Add Text
          </button>
          ...
          <button
            onClick={addTextBox}
            className="w-full bg-gray-700 text-white px-4 py-2 rounded-lg hover:bg-gray-800 text-sm font-mono font-medium transition-colors"
          >
            📝 Add Text Box
          </button>
          ...
          <button
            ...
            className="w-full bg-gray-600 text-white px-4 py-2 rounded-lg hover:bg-gray-700 text-sm font-mono font-medium transition-colors"
          >
            ⬜ Add Rectangle
          </button>
          ...
          <button
            onClick={addCircle}
            className="w-full bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 text-sm font-mono font-medium transition-colors"
          >
            ⚫ Add Circle
          </button>
        </div>
        ...
        <div className="border-t pt-3 space-y-2">
          ...
          <button
            ...
            className="w-full bg-white border-2 border-gray-800 text-gray-800 px-4 py-2 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-mono font-medium transition-colors"
          >
            📋 Duplicate
          </button>
          ...
          <button
            ...
            disabled={!selectedObject}
            className="w-full bg-white border-2 border-gray-700 text-gray-700 px-4 py-2 rounded-lg hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed text-sm font-mono font-medium transition-colors"
          >
            ⬆️ Bring Forward
          </button>
          ...
          <button
            ...
            disabled={!selectedObject}
            className="w-full bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-black disabled:opacity-50 disabled:cursor-not-allowed text-sm font-mono font-medium transition-colors"
          >
            🗑️ Delete
          </button>
        </div>
        ...
        <div className="border-t pt-3 mt-4">
          ...
          <button
            onClick={exportPNG}
            className="w-full bg-gray-800 text-white px-4 py-2 rounded-lg hover:bg-gray-900 mb-2 text-sm font-mono font-medium transition-colors"
          >
            📥 Export PNG
          </button>
          ...
          <button
            onClick={exportJPEG}
            className="w-full bg-gray-700 text-white px-4 py-2 rounded-lg hover:bg-gray-800 mb-2 text-sm font-mono font-medium transition-colors"
          >
            📥 Export JPEG
          </button>
          ...
          <button
            onClick={exportPDF}
            className="w-full bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-black text-sm font-mono font-medium transition-colors"
          >
            📥 Export PDF
          </button>
        </div>
      ...
      <div className="flex-1 flex items-center justify-center bg-gradient-to-br from-gray-100 to-gray-200 p-8">
        <div className="bg-white shadow-2xl rounded-lg overflow-visible" style={{ pointerEvents: 'auto' }}>
          <canvas 
            ref={canvasRef} 
            style={{ 
              display: 'block', 
            ...
            <div className="space-y-3">
              ...
              {selectedObject.type === 'image' ? (
                <div className="space-y-3">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button
                        ...
                        className="px-3 py-2 text-xs bg-white border-2 border-gray-800 text-gray-800 rounded hover:bg-gray-50 font-medium"
                      >
                        None
                      </button>
                      <button
                        onClick={() => applyFilter('grayscale')}
                        className="px-3 py-2 text-xs bg-gray-200 border border-gray-400 text-gray-800 rounded hover:bg-gray-300 font-medium"
                      >
                        Grayscale
                      </button>
                      <button
                  ...
                  <button
                    onClick={cropImage}
                    className="w-full px-3 py-2 bg-gray-800 text-white rounded-lg hover:bg-gray-900 text-sm font-medium"
                  >
                    ✂️ Crop Image
                  </button>
                </div>
